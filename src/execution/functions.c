...
static struct ast *redir_dup(struct ast *r) // duplicates a redirection AST list
{
    ...
    {
        struct ast *n = ast_redir_init(r->data.ast_redir.io_number,
                                       r->data.ast_redir.type,
                                       r->data.ast_redir.word, NULL);
        if (!n) return ast_free(head), NULL;
        if (!head) head = n;
        else tail->data.ast_redir.next = n;
        tail = n;
    }
    return head;
}
...
static struct ast *list_dup(struct ast *l) // duplicates a list AST
{
    ...
    while (l && l->type == AST_LIST)
    {
        ...
        if (!nn) return ast_free(head), NULL;
        if (!head) head = nn;
        else tail->data.ast_list.next = nn;
        tail = nn;
...
{
    if (!n) return NULL;
    if (n->type == AST_LIST) return list_dup(n);
    if (n->type == AST_CMD) return cmd_dup(n);
    if (n->type == AST_IF)
    ...
    if (n->type == AST_REDIRWRAP)
        return ast_redirwrap_init(ast_dup(n->data.ast_redirwrap.shell_command),
...
int functions_register(const struct ast *funcdec) // registers a new function from a FUNCDEC AST node
{
    if (!funcdec || funcdec->type != AST_FUNCDEC) return 1;
    ...
    if (!body && funcdec->data.ast_funcdec.body) return 1;
    if (ex)
    {
        ast_free(ex->body); ast_free(ex->redirs);
        ex->body = body; ex->redirs = rd;
        return 0;
    }
    struct sh_function *n = calloc(1, sizeof(*n));
    if (!n) return ast_free(body), ast_free(rd), 1;
    n->name = xstrdup(funcdec->data.ast_funcdec.name);
    n->body = body; n->redirs = rd;
    n->next = g_funcs; g_funcs = n;
    return 0;
}
...
{
    int n = 0; char k[16];
    while (1) { sprintf(k, "%d", n + 1); if (!hash_map_get(hm, k)) break; n++; }
...
static void pos_restore(struct hash_map *hm, char **old) // restores positional parameters to the hash map
{
    pos_clear(hm);
...
static void pos_set_from_argv(struct hash_map *hm, char **argv) // sets positional parameters in the hash map from argv
{
    pos_clear(hm);
...
static int redirs_apply_both(struct sh_function const *fn, struct fn_call *call,
                            struct saved_fd **saved) // applies redirections for both function definition and call
{
    if (apply_redirs(fn->redirs, saved) != 0) return 1;
    if (apply_redirs(call->redirs, saved) != 0) return 1;
    return 0;
}

int exec_function_call(const struct sh_function *fn, struct fn_call *call) // executes a function call
{
